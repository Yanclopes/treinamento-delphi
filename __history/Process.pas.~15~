...
var
  ...
  sDiretorioBaseExe, sRaizDoProjeto, sPastaCapitulo, sSubPastaRelativa: string;
begin
  ...
  sRaizDoProjeto := ExtractFilePath(ExcludeTrailingPathDelimiter(
                       ExtractFilePath(ExcludeTrailingPathDelimiter(sDiretorioBaseExe))
                     ));

  sPastaCapitulo := IntToStr(sCapitulo) + '.' + IntToStr(sNumero);
  sSubPastaRelativa := sPastaCapitulo + '\Release\Win32';
